...
export function MarkdownContent({ source, className, paragraphClassName }: MarkdownContentProps) {
  ...
  const prepared = source.replace(/<svg[\s\S]*?<\/svg>/gi, (m) => m.replace(/\r?\n/g, ' '))

  // Mirror the TOC's slug-dedup so heading ids match its hrefs (shared implementation).
  ...
  const heading =
    ...
    ({ node, children }: { node?: Element; children?: ReactNode }) => (
      <div id={node ? nextHeadingId(hastText(node).trim()) : undefined} className="scroll-mt-20">
      ...
      <div className={cn(blockquoteStyles, anchorStyles, codeStyles, className, 'w-full [&>*]:mb-6')}>
        <Markdown
          ...
          components={{
            ...
            ol: ({ children }) => (
              <ol className="my-2 list-decimal list-outside space-y-1 pl-6 ml-1">{children}</ol>
            ),
