...
Notice what happened to the constraint. In the constraint bypass lesson, the move was to make a limiting rule irrelevant instead of fighting it. Here the rule "someone must move first" looked unavoidable. Wrapping both transfers into one unit does not satisfy that rule or soften its consequences. It makes the rule irrelevant, because with no first move there is no first mover left to be exposed.
