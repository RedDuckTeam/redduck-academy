...
Ethereum's CREATE2 applies content-derived identity to deploying contracts. A contract's address is derived from the address of the deployer, a chosen value called a salt, and the hash of the contract's bytecode. Because the address is a function of those inputs, it can be computed before the contract exists and verified afterward without consulting any registry. Uniswap v2 relies on this, so anyone can compute the address of the pair contract for two tokens directly from the two token addresses and check that the contract living at that address is the real one. The ERC8009 design, examined in depth in the ERC8009 course, gives its single shared contract one deterministic canonical address on each chain, which is what lets hardware wallet firmware hardcode that address and verify it.
