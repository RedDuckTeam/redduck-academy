...
Compressed NFTs, or cNFTs, solve this with state compression. Instead of storing each NFT's data in its own account, the entire collection is represented as a Merkle tree. Only the tree's root hash lives on chain, in a single account that can hold up to millions of leaves depending on its configured depth. The actual NFT data, such as names, URIs, and owners, lives off chain in indexers. When you want to prove you own a specific cNFT, you provide a Merkle proof against the on-chain root.
